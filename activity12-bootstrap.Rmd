...
```{r}
gss %>% 
  ggplot(mapping = aes(x = educ, y = ..density..)) +
  geom_histogram() +
...
The center of the distribution is around 13/14.  There seems to be a bimodel distribution with a peak around 12 and another around 16.
...
I would argue that the mean is not a good statistic to describe the typical value of salary as most data is distributed on either side of the mean (bimodel).
...
I would argue that due to the CLT, a sample statistic will conform to an approximately normal distribution with a large number of samples (no matter what the distribution is of the population).  Thus, we can reasonably assume the distributio of the sample mean is normal.
...
We are 90% confident that the true population education years mean falls between 13.65 and 13.83 year.
...
I believe the median is the best statistic to describe the number of years of education.  This is because we are able to deterine a much more precise confidence interval (13.65, 13.82) vs (13, 14) or (14, 14).
...
We are 90% confident that the true population proportion of proportion differences between liberals and conservatives who support science research is between 35.68 and 41.17 percent.
